...
public static class StackOverflowClientExtensions
{
    public static IServiceCollection AddStackOverflowClient(this IServiceCollection services, IConfiguration configuration)
    {
        services.AddHttpClient<IStackOverflowClient, StackOverflowClient>((sp, client) =>
        {
                ...
                .AdvancedCircuitBreakerAsync(
                    ...
                    onBreak: (outcome, duration) =>
                    {
                        if (outcome.Exception != null)
                        {
                            logger.LogWarning(
                                outcome.Exception,
                                "StackOverflow circuit breaker OPEN for {Duration} seconds because of exception",
                                duration.TotalSeconds);
                        }
                        else
                        {
                            logger.LogWarning(
                                "StackOverflow circuit breaker OPEN for {Duration} seconds because of status code {StatusCode}",
                                duration.TotalSeconds,
                                (int?)outcome.Result?.StatusCode);
                        }
                    },
                    ...
                    onReset: () =>
                    {
                        logger.LogInformation(
                            "StackOverflow circuit breaker CLOSED");
                    },
